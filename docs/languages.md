...
| `spaces`                | Whether spaces separate the words. Taken from `script` when missing: `false` for a language whose script is written without them (kana, Chinese characters, Thai, Lao, Tibetan, Myanmar, Khmer), `true` otherwise. Where they do separate words, pointing anywhere in a word looks up the whole word.                                                                   |
...
| `example_sentence`      | An everyday sentence, used to test translation (Health) and to try the popup on in the first-run setup; `sample_text` when missing.                                                                                                                   |
...
| `translation`           | The OPUS-MT model sentences are translated into English with: the Hugging Face `model` holding its ONNX export (as Xenova's do), the `revision` (a commit) to download, and its `size` and `full_size` in bytes (quantized, and with full weights). Languages can share one. |
| `distinctive_letters`   | Letters only this language uses among those sharing its script, such as `іїєґ` for Ukrainian, so a sentence is translated with the right language's model.                                                                                            |
